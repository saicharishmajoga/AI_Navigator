...
async def ask_rag_stream(query: str):
    """
    Search ChromaDB for relevant AI tools, enrich context with active SQL database records,
    construct a grounded prompt matching user guidelines, and yield streaming chunks.
    """
    ...
    # 3. Build grounded vector context
    context_chunks = []
    sources = []
    
    for doc in docs:
        meta = doc.metadata or {}
        tool_name = meta.get("tool_name", "AI Tool")
        version = meta.get("version", "N/A")
        pricing = meta.get("pricing", "Free")
        website = meta.get("source", "")
        
        context_chunks.append(
            f"Tool: {tool_name}\n"
            f"Category: {meta.get('category', 'AI Tools')}\n"
            f"Pricing: {pricing}\n"
            f"Version: {version}\n"
            f"Website: {website}\n"
    ...
    # Compile the active project database directory
    db_tools_context = []
    ...
    # 4. Formulate the advanced system instruction matching user discovery, fallback & comparison modes
    system_prompt = (
        ...
        "2. Before stating that information is unavailable, search all retrieved contexts, registered SQL tool lists, aliases, and dependencies.\n"
        ...
        "4. If the exact tool is not documented inside the retrieved contexts or database directory, use your general knowledge and reasoning to answer. Clearly state:\n"
        ...
        f"=== RETRIEVED CHROMA DOCUMENTATION ===\n{context_str}\n\n"
